...
class TextLSTM(nn.Module):
    def __init__(self):
        ...
        self.lstm = nn.LSTM(input_size=input_shape[1] * input_shape[2], hidden_size=hidden, num_layers=num_layers).to(
            device)
...
with torch.no_grad():
    ...
    for i in tqdm(l):
        ...
        model = torch.load(f'model/0_{i}.pt')
        if pred is None :
            pred = model((hid, cell), X_test.to(device)).cpu().detach().numpy() / len(l)
        else :
            pred += model((hid, cell), X_test.to(device)).cpu().detach().numpy() / len(l)
    ...
    y_test.to_csv("test_output_0907.csv", index=False)
